...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-[#07090e] text-gray-100 flex flex-col font-sans selection:bg-violet-500 selection:text-black print:bg-white print:text-black">
      ...
      <header className="bg-[#0b0f19] border-b border-gray-800/80 sticky top-0 z-40 px-4 sm:px-6 py-4 print:hidden">
        <div className="max-w-7xl mx-auto flex flex-col sm:flex-row justify-between items-center gap-4">
          ...
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 bg-gradient-to-tr from-violet-500 to-violet-400 p-[1.5px] rounded-md shadow-lg shadow-violet-500/5">
              <div className="w-full h-full bg-[#07090e] flex items-center justify-center rounded-md">
                <span className="font-mono text-xl font-bold tracking-tighter text-violet-500">▲</span>
              </div>
            </div>
            <div>
              <div className="flex items-center gap-1.5">
                <h1 className="font-bold text-white tracking-tight text-base sm:text-lg">
                  PIRAMID <span className="text-violet-500 text-xs border border-violet-500/30 px-1 py-0.2 rounded font-mono uppercase">ENERGY GOVERNANCE</span>
                </h1>
                <span className="text-[10px] bg-violet-500/10 text-violet-500 border border-violet-500/20 px-1.5 py-0.2 rounded font-mono uppercase font-bold tracking-wider">
                  v2.26
                </span>
              </div>
          ...
          <div className="flex items-center flex-wrap gap-3 sm:gap-4 justify-center">
              ...
              {(['PT', 'EN', 'ES'] as Language[]).map(lang => (
                <button
                  ...
                  className={`px-1.5 py-0.5 rounded transition ${
                    language === lang ? 'bg-violet-500 text-black font-extrabold' : 'text-gray-400 hover:text-white'
                  }`}
                >
                  {lang}
                </button>
            ...
            <button
              ...
              className="px-2.5 py-1.5 border border-violet-500/20 bg-violet-500/10 text-violet-500 hover:bg-violet-500 hover:text-black rounded-lg text-xs font-mono font-bold uppercase transition flex items-center gap-1.5 cursor-pointer"
            ...
            <button
              ...
              className="p-2 border border-gray-800/80 hover:bg-violet-950/10 text-gray-400 hover:text-violet-400 rounded-lg transition"
              title={t.logout}
            >
            ...
            return (
              <button
                ...
                className={`py-1.5 px-3 sm:px-4 rounded-md text-xs font-semibold tracking-wide transition-all whitespace-nowrap flex items-center gap-1.5 cursor-pointer ${
                  isActive
                    ? 'bg-violet-500 text-black font-extrabold shadow-md shadow-violet-500/10'
          ...
          <div className="mb-4 bg-gray-950/60 p-2 border border-gray-800 rounded-lg flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2 print:hidden">
            ...
            {selectedProject && (
              <span className="text-[10px] text-gray-500 font-mono">
                Cliente correspondente: <span className="text-violet-500 font-bold">{selectedProject.client}</span>
              </span>
            )}
          </div>
        )}
