    ...
    <div className="space-y-6">
      ...
      <section className="bg-gray-900/40 p-4 border border-gray-800 rounded-lg flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h2 className="text-base font-bold text-white uppercase tracking-wider font-mono flex items-center gap-2">
            <HeartHandshake className="w-5 h-5 text-amber-500 animate-pulse" />
            {t.crmTitle}
          </h2>
        ...
        <button
          ...
          className="px-3 py-1.5 bg-amber-500 hover:bg-amber-600 text-black text-xs font-semibold rounded flex items-center gap-1 cursor-pointer"
        >
      ...
      <section className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-7 space-y-4">
          ...
          {/* Add contact Form */}
          {isAddingContact && (
            <form onSubmit={handleAddContactSubmit} className="bg-gray-900/60 border border-amber-500/50 p-5 rounded-lg space-y-4">
              <h3 className="text-xs font-bold text-amber-500 font-mono uppercase">
              ...
              <div className="flex justify-end gap-2 text-xs pt-1">
                ...
                <button
                  type="submit"
                  className="px-3 py-1 bg-amber-500 hover:bg-amber-600 rounded text-black font-semibold animate-pulse"
                >
              ...
              >
                <div className="flex justify-between items-start">
                  <div>
                    ...
                    <p className="text-[10px] text-amber-500/90 mt-0.5 flex items-center gap-1 font-sans">
        ...
        <div className="lg:col-span-5 bg-gray-900/20 border border-gray-850 p-5 rounded-lg space-y-4">
          ...
          <form onSubmit={handleAddActivitySubmit} className="space-y-3 bg-[#111827]/60 p-3.5 border border-gray-800 rounded-lg">
            ...
            <div className="flex justify-between items-center gap-2 pt-1.5">
              <div className="flex border border-gray-850 rounded overflow-hidden text-[10px] font-mono bg-[#0a0f18]">
                {[
                  ...
                  <button
                    ...
                    className={`px-2 py-1 ${
                      logType === type.id ? 'bg-amber-500 text-black font-bold' : 'text-gray-400 hover:text-white'
                    }`}
                  >
                    {type.label}
                  </button>
              ...
              <button
                type="submit"
                className="px-2.5 py-1 bg-amber-500 hover:bg-amber-600 text-black text-xs font-bold rounded flex items-center gap-1 cursor-pointer transition-all active:scale-95"
              >
          ...
          <div className="space-y-4 max-h-80 overflow-y-auto pr-1">
            {activities.map((act) => (
              ...
              >
                {/* Node dot sign */}
                <span className="absolute -left-1.5 top-1 w-2.5 h-2.5 rounded-full bg-amber-500 border border-[#0a0f18] animate-pulse" />
