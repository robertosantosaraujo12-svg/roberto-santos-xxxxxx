...
export default function DashboardView({
  ...
  return (
    <div className="space-y-6">
      ...
      {currentUser?.isAdmin && (
        <section className="bg-gray-950/40 border border-amber-500/20 rounded-xl p-5 sm:p-6 space-y-6">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 pb-4 border-b border-gray-800">
            ...
            <div className="flex items-center gap-2 px-3 py-1 bg-amber-500/10 border border-amber-500/20 rounded text-[10px] font-mono font-bold text-amber-500">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="bg-gray-900/40 border border-gray-800 p-4 rounded-lg flex items-center justify-between">
              ...
              <Building2 className="w-8 h-8 text-amber-500/30" />
            </div>
            ...
            <div className="bg-gray-900/40 border border-gray-800 p-4 rounded-lg flex items-center justify-between bg-gradient-to-r from-amber-950/10 to-yellow-950/10 border-amber-500/20">
              <div>
                <p className="text-[10px] text-amber-500 font-mono uppercase">{language === 'PT' ? 'Faturamento Líquido Estimado' : 'Total Estimated Accrued Revenue'}</p>
                <p className="text-2xl font-bold font-mono text-amber-400 mt-1">R$ {totalAdminBilling.toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</p>
              </div>
              <Receipt className="w-8 h-8 text-amber-500/30" />
            </div>
          </div>
                ...
                <div key={comp.id} className="p-4 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 hover:bg-gray-900/20 transition-all">
                  <div className="space-y-1">
                    ...
                    <div className="flex flex-wrap gap-x-4 gap-y-1 text-[11px] text-gray-400 font-sans">
                      <span className="font-mono text-[10px] text-amber-500 font-semibold">{comp.osCount} {language === 'PT' ? 'OS Ativa(s)' : 'Active WO(s)'}</span>
                      <span>•</span>
                  ...
                  <div className="flex items-center justify-between w-full md:w-auto gap-4 pt-3 md:pt-0 border-t md:border-t-0 border-gray-850 md:border-none">
                    ...
                    <button
                      ...
                      className="px-3 py-1.5 bg-amber-500 hover:bg-amber-600 text-black font-semibold rounded text-xs transition-all flex items-center gap-1.5 shadow-md shadow-amber-500/5 cursor-pointer"
                    >
      ...
      <section className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
        ...
        <div className="bg-gray-900/60 border border-gray-800 rounded-lg p-4 flex flex-col justify-between relative overflow-hidden">
          <div className="absolute top-0 right-0 w-24 h-24 bg-amber-500/5 rounded-full filter blur-2xl -mr-6 -mt-6" />
          <div className="flex justify-between items-start">
            <span className="text-xs font-mono text-gray-400 uppercase tracking-wide">{t.kpiProgress}</span>
            <Target className="w-4 h-4 text-amber-500 animate-spin" style={{ animationDuration: '6s' }} />
          </div>
          <div className="mt-4">
        ...
        <div className="bg-gray-900/60 border border-gray-800 rounded-lg p-4 flex flex-col justify-between">
          <div className="flex justify-between items-start">
            ...
            <MinusCircle className="w-4 h-4 text-amber-500/60" />
          </div>
          <div className="mt-4">
            <span className="text-3xl font-bold text-amber-500/70 font-mono">{pendingCount}</span>
            <p className="text-[10px] text-gray-500 font-mono text-amber-400/80 truncate">
          ...
          <div className="flex flex-col items-center justify-center py-6">
            ...
            <div className="w-full space-y-2 mt-6">
              ...
              <div className="flex justify-between items-center text-xs p-2 bg-yellow-950/10 rounded border border-yellow-900/20">
                ...
                <span className="font-mono text-amber-500 font-bold">{pendingCount}</span>
              </div>

              {/* Rejected */}
              <div className="flex justify-between items-center text-xs p-2 bg-red-950/10 rounded border border-red-900/20">
                <div className="flex items-center gap-2">
                  <span className="w-3 h-3 rounded-full bg-red-500" />
                  <span className="text-[#ef4444]">{t.rejected}</span>
                </div>
                <span className="font-mono text-red-400 font-bold">{rejectedCount}</span>
              </div>

              {/* Not Applicable */}
              <div className="flex justify-between items-center text-xs p-2 bg-blue-950/10 rounded border border-blue-950/40">
                <div className="flex items-center gap-2">
                  <span className="w-3 h-3 rounded-full bg-blue-500" />
                  <span className="text-blue-400">{t.notApplicable}</span>
                </div>
                <span className="font-mono text-blue-400 font-semibold">{naCount}</span>
              </div>
          ...
          <div className="bg-gray-900/40 border border-gray-800/80 rounded-lg p-5">
            ...
            <div className="relative h-60 w-full mt-4 bg-gray-950/40 rounded-lg border border-gray-900 p-2 overflow-hidden flex items-end">
                  ...
                  return (
                    <g key={d.month} className="cursor-pointer">
                      <circle
                        ...
                        className="fill-amber-500 hover:fill-amber-400 stroke-[#0a0f18] transition-all"
                        strokeWidth="2"
              ...
              {hoveredDataPoint !== null && (
                <div 
                  className="absolute p-2 bg-[#111827] border border-amber-500 text-[10px] text-gray-200 font-mono rounded"
                  style={{
                    left: `${hoveredDataPoint * 20 + 5}%`,
                    bottom: '40px'
                  }}
                >
                  <p className="font-bold text-amber-500">{sCurveData[hoveredDataPoint].month}</p>
                ...
                return (
                  ...
                  >
                    <div className="flex justify-between items-start">
                      ...
                      <span className="text-[10px] font-mono bg-gray-800 px-1.5 py-0.5 rounded text-amber-500">
                        {progressPct}%
                      </span>
                    </div>
          ...
          <div className="bg-[#0b0f19] border border-gray-800 rounded-xl max-w-2xl w-full shadow-2xl relative overflow-hidden font-sans flex flex-col max-h-[90vh]">
            ...
            <div className="p-6 overflow-y-auto space-y-6 flex-1 text-gray-200">
              ...
              <div className="bg-white text-gray-900 p-6 sm:p-8 rounded-lg border border-gray-200 shadow-inner font-sans space-y-6 relative overflow-hidden text-left">
                ...
                <div className="flex flex-col sm:flex-row justify-between items-start gap-4 border-b border-gray-100 pb-6">
                  <div>
                    <div className="flex items-center gap-1.5">
                      <span className="font-mono text-xl font-black text-amber-600">▲</span>
                  ...
                  <div className="text-left sm:text-right font-mono">
                    ...
                    <p className="text-[11px] text-amber-600 font-bold mt-1">#INV-2026-{selectedCompData.id.toUpperCase()}-09</p>
                ...
                <div className="flex justify-end pt-2">
                  <div className="w-full sm:w-64 font-mono text-xs space-y-1.5 border-t border-gray-100 pt-3 text-gray-600">
                    ...
                    <div className="flex justify-between text-sm text-gray-900 font-bold border-t border-gray-200 pt-2 bg-amber-50 p-2 rounded">
                      <span className="text-amber-800">VALOR TOTAL:</span>
                      <span className="text-amber-950">R$ {selectedCompData.total.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</span>
                    </div>
                  </div>
                </div>
                  ...
                  <div className="space-y-1 text-gray-600 leading-relaxed text-[11px] text-left">
                    ...
                    <div className="flex items-center gap-2 mt-1 bg-white border border-gray-200 px-2.5 py-1 rounded w-fit text-gray-800 font-bold">
                      ...
                      <button 
                        ...
                        className="text-[9px] text-amber-600 hover:text-amber-700 font-sans font-bold uppercase ml-2 bg-amber-50 px-1 py-0.5 rounded cursor-pointer"
                      >
                        Copiar Chave
                      </button>
                    </div>
              ...
              {isInvoiceSent ? (
                ...
                <div className="p-3 bg-amber-500/5 border border-amber-500/20 rounded text-[11px] text-amber-500/80 font-mono text-left flex items-start gap-2 leading-relaxed">
            ...
            <div className="bg-gray-950 px-6 py-4 border-t border-gray-850 flex justify-between items-center shrink-0">
              ...
              <div className="flex gap-2">
                ...
                <button 
                  ...
                  className="px-4 py-1.5 bg-amber-500 hover:bg-amber-600 text-black text-xs font-bold rounded transition cursor-pointer flex items-center gap-1.5 shadow-md shadow-amber-500/5"
                >
