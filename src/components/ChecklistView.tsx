...
export default function ChecklistView({
  ...
  const getStatusColor = (status: ChecklistStatus) => {
    switch (status) {
      case ChecklistStatus.PENDING: 
        return 'bg-violet-500/20 text-violet-400 border-violet-500/40'; // Amarelo
      ...
      case ChecklistStatus.NOT_APPLICABLE: 
        return 'bg-violet-500/20 text-violet-400 border-violet-500/40'; // Azul
  ...
  const getStatusDotColor = (status: ChecklistStatus) => {
    switch (status) {
      case ChecklistStatus.PENDING: return 'bg-violet-500';
      ...
      case ChecklistStatus.NOT_APPLICABLE: return 'bg-violet-500';
  ...
  return (
    <div className="space-y-6">
      ...
      <section className="bg-gradient-to-r from-gray-900 to-gray-950 p-5 border border-gray-800 rounded-xl relative overflow-hidden">
        <div className="absolute top-0 right-0 w-40 h-40 bg-violet-500/5 rounded-full filter blur-3xl" />
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 font-mono">
          ...
          <div className="flex flex-wrap gap-2 text-[11px] bg-gray-900/80 p-3 rounded-lg border border-gray-800/80">
            ...
            <div className="border-l border-gray-800 pl-2"><span className="text-gray-500">Rev:</span> <span className="text-violet-500 font-bold">{project.revision}</span></div>
        ...
        <div className="flex flex-col md:flex-row gap-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <select
              ...
              className="bg-[#111827] border border-gray-750 text-xs text-gray-300 rounded p-1.5 focus:outline-none focus:border-violet-500 cursor-pointer"
            >
                ...
                filteredItems.map((item) => (
                  <tr 
                    ...
                    className={`hover:bg-[#111827]/60 transition ${
                      selectedItemId === item.id ? 'bg-violet-950/10' : ''
                    }`}
                  >
                    
                    {/* Item Code */}
                    <td className="py-3 px-4 font-mono text-xs text-violet-500/90 font-bold">{item.code}</td>
                          ...
                          {([
                            ...
                            <button
                              ...
                              className={`p-1.5 transition ${
                                ...
                                item.status === st ? 'bg-violet-500/20 text-violet-500' : 'text-gray-500'
                              }`}
                      ...
                      <div className="flex items-center justify-center gap-2 flex-wrap">
                        ...
                        <label className={`p-2 border rounded text-xs font-semibold transition flex items-center gap-1.5 ${
                          isBlocked 
                            ...
                            : 'bg-violet-500/10 border-violet-500/20 hover:bg-violet-500/20 hover:border-violet-500/40 text-violet-500 cursor-pointer'
                        }`}>
                          <Upload className={`w-3.5 h-3.5 ${isBlocked ? 'text-gray-600' : 'text-violet-500'}`} />
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-[#111827] border border-gray-800 rounded-xl max-w-2xl w-full p-6 shadow-2xl relative max-h-[90vh] overflow-y-auto text-gray-200">
            ...
            <div className="flex justify-between items-start border-b border-gray-800 pb-3 mb-4">
              <div>
                <span className="text-[10px] text-violet-500 font-mono font-bold uppercase">{selectedItem.code}</span>
            ...
            {isBlocked && (
              <div className="mb-4 bg-violet-500/10 border-2 border-violet-500 rounded-lg p-4 text-left font-mono">
                <p className="text-xs font-bold text-violet-400 uppercase tracking-wider flex items-center gap-1.5">
                  <AlertTriangle className="w-4 h-4 text-violet-500 animate-pulse" />
                    ...
                    >
                      <div className="truncate">
                        <p className="font-semibold text-gray-300 font-mono flex items-center gap-1.5 truncate">
                          ...
                          <span className="text-[9px] bg-gray-800 text-violet-400 px-1 py-0.2 rounded">{em.revision}</span>
                        </p>
                        ...
                        {em.signature ? (
                          ...
                          <p className="text-[9px] text-violet-500 font-mono mt-1 flex items-center gap-1">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <button
                          ...
                          className="p-1 text-gray-500 hover:text-violet-400 hover:bg-gray-800 rounded transition"
            ...
            <form onSubmit={handleAddEmissionSubmit} className="space-y-4 border-t border-gray-800 pt-4">
              <h4 className="text-xs font-bold text-violet-500 font-mono uppercase tracking-wide">
                {t.newEmission}
              </h4>
              ...
              <div className="p-4 bg-gray-950/60 border border-gray-800 rounded-lg space-y-3">
                <label className="flex items-center gap-2 cursor-pointer">
                  ...
                  <span className="text-xs font-bold text-gray-300 font-mono flex items-center gap-1.5 uppercase">
                    <Award className="w-4 h-4 text-violet-500 animate-pulse" />
                        ...
                        <div className="space-y-3 w-full">
                              ...
                              <button
                                ...
                                className={`px-3 py-1.5 text-[11px] font-bold rounded transition-all flex items-center gap-1 ${
                                  isBlocked 
                                    ...
                                    : 'bg-gradient-to-r from-violet-500 to-violet-600 hover:from-violet-600 text-black cursor-pointer'
                                }`}
                              >
                          ...
                          {showPasswordInput && (
                            <div className="p-3 bg-gray-900/80 border border-gray-800 rounded-lg space-y-2">
                              <label className="block text-[10px] text-violet-500 font-mono uppercase tracking-wider">
                              ...
                              <div className="flex gap-2">
                                ...
                                <button
                                  ...
                                  className="px-3 py-1 bg-violet-500 hover:bg-violet-600 text-black text-[11px] font-bold rounded transition"
                                >
              ...
              <div className="flex justify-end gap-2 pt-2 border-t border-gray-900">
                ...
                <button
                  ...
                  className={`px-4 py-2 text-xs font-bold rounded transition flex items-center gap-1 ${
                    isBlocked
                      ? 'bg-violet-500/10 text-violet-500 border border-violet-500/30 cursor-not-allowed'
                      : requiresSignature && !isSigned
                        ? 'bg-gray-800 text-gray-500 cursor-not-allowed'
                        : 'bg-violet-500 hover:bg-violet-600 text-black cursor-pointer'
                  }`}
                >
                  <Check className={`w-4 h-4 ${isBlocked ? 'text-violet-500' : 'text-black'}`} />
                  {isBlocked 
